...
from udfs import date_udfs, df_udfs, html_udfs, misc_udfs
...
start_dt='01-01-2000'
end_dt='12-31-2040'
cal_lat = 9.0  #set locale latitude;  addis, tx is lat 9.0, lon 38.7
cal_lon = 38.7 #set locale longitude; addis, tx is lat 9.0, lon 38.7
...
# Apply the convert_to_ethiopian function to the 'dt' column and create a new 'eth_dt' column
df['eth_dt'] = df['dt'].apply(date_udfs.convert_to_ethiopian)

#year as int
df['y'] = pd.DatetimeIndex(df['dt']).year
df['eth_y'] = df['eth_dt'].apply(date_udfs.extract_year)
#month as int
df['m'] = pd.DatetimeIndex(df['dt']).month
df['eth_m'] = df['eth_dt'].apply(date_udfs.extract_month)

#calendar day as int
df['d'] = pd.DatetimeIndex(df['dt']).day
df['eth_d'] = df['eth_dt'].apply(date_udfs.extract_day)

#yearmonth as int
df['ym'] = df['y']*100 + df['m']
df['eth_ym'] = df['eth_y']*100 + df['eth_m']

#date in yyyymmdd as int
df['dt_int'] = df['y']*10000 + df['m']*100 + df['d']
df['eth_dt_int'] = df['eth_y']*10000 + df['eth_m']*100 + df['eth_d']
...
df['is_holiday'] = df['dt'].apply(date_udfs.check_eth_holiday)
df['holiday'] = df['dt'].apply(date_udfs.get_eth_holiday_label)
